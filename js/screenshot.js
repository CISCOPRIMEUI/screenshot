...
window.Feedback = function( options ) {
    ...
    // default properties
    options.label = options.label || "Take Screenshot";
    options.header = options.header || "Take Screenshot";
    options.url = options.url || "/";
    options.adapter = options.adapter || new window.Feedback.XHR( options.url );
    
    options.nextLabel = options.nextLabel || "Continue";
    options.reviewLabel = options.reviewLabel || "Save";
    options.saveLabel = options.saveLabel || "Save";
    options.closeLabel = options.closeLabel || "Close";
    
    options.messageSuccess = options.messageSuccess || "Your screenshot was saved succesfully.";
    options.messageError = options.messageError || "Your screenshot was not saved, please reload page and try again.";
    ...
    returnMethods = {
        open: function() {
            ...
            nextButton.onclick = function() {   
                ...
                if ( currentPage === len ) {
                    returnMethods.save( options.adapter );
                } else {
                    ...
                    // if last page, change button label to save
                    if ( currentPage === len ) {
                        nextButton.firstChild.nodeValue = options.saveLabel;
                    }
        ...
        save: function( adapter ) {
            ...
            // make sure save adapter is of right prototype
            if ( !(adapter instanceof window.Feedback.Save) ) {
                throw new Error( "Adapter is not an instance of Feedback.Save" );
            }
            ...
            //  data to adapter for processing
            adapter.save( data, function( success ) {
...
window.Feedback.Save = function() {};
window.Feedback.Save.prototype = {
...
window.Feedback.Form = function( elements ) {
    ...
    this.elements = elements || [{
        type: "textarea",
        name: "Issue",
        label: "Please click on the areas where you want to highlight and blackout the areas you do not want to be viewable",
        required: false
    }];
...
window.Feedback.XHR.prototype = new window.Feedback.Save();

window.Feedback.XHR.prototype.save = function( data, callback ) {
    ...
    xhr.save( "data=" + encodeURIComponent( window.JSON.stringify( data ) ) );
